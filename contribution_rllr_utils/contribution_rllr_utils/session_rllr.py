...
def compute_session_rllr(
    ...
    *,
    ...
    contrib_rscc: Dict[str, Dict[int, float]] = {v: {} for v in VARS_ALL}
    ...
    for ni, full_vars in pyr_models.items():
        ...
        if need_llhi:
            deltas = np.array([contrib_delta_llhi[v].get(ni, np.nan) for v in full_vars], dtype=float)
            finite = np.isfinite(deltas)
            l2_norm = float(np.sqrt(np.sum(np.square(deltas[finite])))) if np.any(finite) else 0.0
            for v in VARS_ALL:
                if v in full_vars:
                    delta_v = contrib_delta_llhi[v].get(ni, np.nan)
                    if np.isfinite(delta_v) and np.isfinite(l2_norm) and l2_norm > MU_EPS:
                        contrib_rscc[v][ni] = float(delta_v / l2_norm)
                    else:
                        contrib_rscc[v][ni] = 0.0
                else:
                    contrib_rscc[v][ni] = 0.0
